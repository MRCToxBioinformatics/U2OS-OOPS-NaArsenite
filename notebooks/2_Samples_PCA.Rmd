...
for(i in list(total_as_protein_quant, oops_as_protein_quant)){
  p = plotSamplePCA(i)
  pdf(paste("../plots/",names[c],"_PCA-on-normalised-data.pdf",sep=""),
...
In this case, Replicate 1 of the Control seems to be very different to the rest of the samples and that's what PC1 is captuting (variation 62%), alongside the difference between Ctrl and Arsenite.

**_2. RBPs : Control vs Arsenite treated_**    
In this case, Replicate 1 of the Control seems to be very different to the rest of the samples and that's what PC1 is captuting (variation 53%). PC1 and PC2 seem to approximately separate the treatment vs control samples (variation 53% & 10%, respectively).

We have no real reason to remove Ctrl 1 so we will keep it in for now. 
